...
class ListOfAtoms:
	...
	def KineticEnergy(self):
		...
		return 0.5*scipy.sum(self.mass*self.velocities*self.velocities)
...
class VisualDisplayAtomsObserver(AtomsObserver):
	def __init__(self, atoms, L, thk=0.3, rate=200, velocityColors=False):
		"""
		...
		# elif dim==2:
		# 	vi.scene.range = (L/2.+thk, L/2.+thk, L/2.+thk)
		vi.scene.center=vi.vector(L/2., L/2., 0.0)
		vi.rate(rate)
		# Create Wall(s)
		sVert = L 
		sHoriz = L+2.*thk
